...
export class HttpService {
  ...
  showTask(id){
    // our http response is an Observable, store it in a variable
    // let tempObservable = this._http.get('/api/showTask', id);
    // subscribe to the Observable and provide the code we would like to do with our data from the response
    // tempObservable.subscribe(data => console.log("Showed our task!", data));
    return this._http.get(`/api/showTask/${id}`);        
  }

  updateTask(id,data){
    // our http response is an Observable, store it in a variable
    // let tempObservable = this._http.put('/api/updateTask',id, data);
    // subscribe to the Observable and provide the code we would like to do with our data from the response
    // tempObservable.subscribe(data => console.log("Updated our task!", data));
    return this._http.put(`/api/updateTask/${id}` ,data);    
    
  }

  deleteTask(id){
    // our http response is an Observable, store it in a variable
    // let tempObservable = this._http.delete('/api/deleteTask',id);
    // subscribe to the Observable and provide the code we would like to do with our data from the response
    // tempObservable.subscribe(data => console.log("Deleted our task!", data));
    return this._http.delete(`/api/deleteTask/${id}`);    
